...
function test_swarm {
    ansible-playbook -i inventory/libnetwork-hosts test.yml -e libnetwork_install_type=swarmkit -e action=deploy
    ...
    if [ $? -ne 0 ]
    then
        echo "fail to list swarm nodes"
        exit 1
    fi

    ansible-playbook -i inventory/libnetwork-hosts test.yml -e libnetwork_install_type=swarmkit -e action=clean
}
